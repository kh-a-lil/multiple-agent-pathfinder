...
class PathFinder:
    ...
    def visualize_tmp(self, filename="simulation.mp4"):
        ...
        def get_node_color(color_name):
            ...
            if c_lower in THEME_COLORS:
                return THEME_COLORS[c_lower]
            try:
        ...
        def get_drone_pos(d_id, T):
            """Linear Interpolation (Lerp) to
                find exact X,Y at fractional time T"""
            t1 = int(T)
        ...
        def draw(frame_idx):
            ...
            for loc, (x, y) in pos.items():
                ...
                ax.text(
                    ...
                    va="top",
                    fontsize=label_font,
                    rotation=25 if is_massive else 0,
                    zorder=3,
                )
            ...
            for (x, y), drones in pos_groups.items():
                ...
                ax.text(
                    ...
                    ha="center",
                    va="bottom",
                    fontsize=7 if is_massive else 10,
                    fontweight="bold",
                    bbox=dict(
                        facecolor="#0F172A",
                        alpha=0.8,
            ...
            ax.set_title(
                f"Simulation Turn {current_turn}",
                color=TEXT,
                fontsize=20,
                pad=20,
                fontweight="bold",
        ...
        anim.save(filename, writer=writer, savefig_kwargs={"facecolor": BG})
